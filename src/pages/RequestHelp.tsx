import { IonButton, IonButtons, IonCard, IonContent, IonHeader, IonInput, IonItem, IonLabel, IonList, IonToast, IonMenuButton, IonPage, IonRow, IonSelect, IonSelectOption, IonTitle, IonToolbar, IonText } from "@ionic/react";
...
const RequestHelp: React.FC = () => {
    ...
    const submitHandler = () => {
        if (namaRef.current?.value === "" || nimRef.current?.value === "" || lineRef.current?.value === "" || orgRef.current?.value === "" || jabatanRef.current?.value === "" || acaraRef.current?.value === "" || tglRef.current?.value === "" || waktuRef.current?.value === "" || jumlahRef.current?.value === "" || deskripsiRef.current?.value === "" || keteranganRef.current?.value === "" || jenisBantuan === "") {
            setNullToast(true);
        } else {
            addData();
        }
                ...
                <IonCard>
                    <form className="ion-padding">
                        ...
                        <IonItem>
                            ...
                            <IonInput ref={jumlahRef} type="number" placeholder="Hanya isi bila memilih jasa, apabila memilih peralatan isi saja dengan 0" />
                        </IonItem>
                        <IonItem>
